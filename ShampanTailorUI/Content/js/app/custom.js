...
function addRow2($table) {
    var $row = $('<tr>');
    var serialNo = $table.find('tbody tr').length + 1;

    $table.find('thead th').each(function () {
        var type = $(this).data('type');
        var isHidden = $(this).is(':hidden');
        var columName = $(this).attr('data-name') || $(this).text();
        columName = columName.replace(/ /g, "-");

        if ($(this).text().trim() === "Action") {
            return true; // Skip Action column
        }
    ...
    // 🔽 Add both delete and add buttons in the Action cell
    var $actionCell = $('<td>');
    var $deleteBtn = $('<button type="button" class="btn btn-danger btn-sm remove-row-btn"><i class="fa fa-trash"></i></button>');
    var $addBtn = $('<button type="button" class="btn btn-success btn-sm add-row-btn"><i class="fa fa-plus"></i></button>');

    $actionCell.append($deleteBtn).append(' ').append($addBtn);
...
function addRowAdvance($table) {
    ...
    $table.find('thead th').each(function () {
        ...
        //}
            ...
            var $checkbox = $('<input>', {
                ...
                'data-size': 'sm',
                'data-type': 'checkbox' // keep for your initEditTable to recognize
            });
            $cell.append($checkbox);

            // After appending, initialize the bootstrap toggle for this checkbox
    ...
    // Focus on first input except checkbox (optional)
    $row.find('input:not([type=checkbox])').first().focus();
...
function serializeTableEnableCheckbox($table) {
    // Get the table headers (column names)
    const headers = $table.find('th').map((index, element) => {
        var name = $(element).attr("data-name");
        if (name) return name;
    ...
    const data = rows.map((index, row) => {
        const cells = $(row).find('td').map((index, cell) => {
            ...
            // If the cell contains a checkbox, get its checked status
            let $checkbox = $cell.find('input[type="checkbox"]');
            if ($checkbox.length) {
                return $checkbox.is(':checked') ? true : false;
            }

            // Otherwise, extract text content and clean numeric values
